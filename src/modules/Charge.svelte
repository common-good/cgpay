...
/*    if ((new RegExp('^[0-9A-Za-z]{12,29}[\.!]$')).test(qr)) { // like H6VM0G0NyCBBlUF1qWNZ2k.
    ...
    if (!testing && u.fakeData()) throw new Error('That is a real Common Good card and cannot be used in test mode.')
    if (testing && !u.fakeData()) throw new Error('That is a CGPay test card and cannot be used in production mode.')
// NO. This risks our data integrity    if (testing != u.fakeData()) changeMode(testing)
